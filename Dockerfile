...
LABEL maintainer="Nadezhda Ryabtsova <nadezhdaryabtsova@gmail.com>, Artem Naumenko <entsupml@gmail.com>"
...
RUN mkdir -p /usr/src/janus /var/janus/janus/log /var/janus/janus/data && \
	cd /usr/src/janus && \
        wget -c https://github.com/meetecho/janus-gateway/archive/v0.6.0.tar.gz && \                                                                                                                        
        tar -xzf v0.6.0.tar.gz && \                                                                                                                                                                         
        cd janus-gateway-0.6.0 && \
	sh autogen.sh && \
